...
function PANEL:Paint( w, h )
	...
	if self.customPaint then
		local Col = self.customPaint
		draw.RoundedBox( 0, 0, 0, w, h, Color( Col.r, Col.g, Col.b, Col.a ) )
		return
	end
	
	if self:IsLineSelected() then
		draw.RoundedBox( 0, 0, 0, w, h, Color( 225, 225, 225, ANUS_MENUALPHA ) )
		return
	end
	
	if self.altLine then
		draw.RoundedBox( 0, 0, 0, w, h, Color( 240, 240, 240, ANUS_MENUALPHA ) )
	end
...
function PANEL:SetColumnIcon( ColumnID, path )
	self.Icons[ ColumnID ] = "icon"
	self.TextSet = self:GetText()
end

function PANEL:SetColumnIconButton( ColumnID, callback )
